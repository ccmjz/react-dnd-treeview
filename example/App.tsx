...
const recursivelyUpdateNode = (
  ...
  nodeUpdateFunc: (node: TestNode, parentNode: TreeNode) => TestNode
) => {
  ...
  return nodeUpdateFunc(node, parentNode);
};
...
const recursivelyUpdateList = (
  ...
  const mappedItems = list.items.map(item =>
    recursivelyUpdateNode(item, parentNode, listUpdateFunc, nodeUpdateFunc)
  );
